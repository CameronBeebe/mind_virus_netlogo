;; CB: Modifying Wilensky's Virus on a Network to instead model memes and mind viruses.
;; CB: Changes should be well marked.
...
[
  ...
  debate              ;; CB: Turtles' willingness to debate. Random integer.  Lower if infected, higher if resistant.
  gullibility         ;; CB: Turtles' gullibility to a given meme or mind virus, independent of debate.  Random integer.
]
...
to setup
  ...
  ;; CB: Begin additional code for resistance.

  ;; CB: Also some non-zero immunity should be possible.
  ;; CB: E.g. a resistance group of critical thinkers.
  ;; CB: Intuition: A small number of robust nodes can have
  ;; a great impact on rate of spread or time to herd immunity.

  if resistance [
    ask n-of initial-resistance-size turtles with [not infected?]
    [ become-resistant ]
  ]
  ;; CB: End resistance addition.


  ;; CB: Set debate parameter.
  ask turtles [set debate random 100]
  ask turtles [set gullibility random 100]
...
to go
  if all? turtles [not infected?]
    [ stop ]
  ;; CB: Might want another stop condition?
  ...
  ;; CB: Add spread-resistance to go loop
  spread-resistance
...
to become-infected  ;; turtle procedure
  ...
  ;; CB: Modify debate when infected.
  ;; CB: When infected with a mind virus, host is less willing to debate.
  set debate debate - random 100
  if debate < 0 [set debate 0]
  ;; CB: End mod.
...
to become-resistant  ;; turtle procedure
  ;; CB: Split logic of become-resistant.
  ;; CB: Those with mind virus are harder to make resistant because of low willingness to debate.
  ;; CB: So, we can add difficulty of becoming resistant dependant on debate factor.
  ;; CB: In other words, the turtles debate!  Or if here from do-virus-checks, critical introspection!
  if infected?
    [if random 101 <= debate [
      set infected? false
      set resistant? true
      set color gray
...
to spread-virus
  ask turtles with [infected?]
    ;; CB: Add gullibility condition.  Even if you are willing to debate, a population can still be gullible to a meme.
    ;; CB: Note, does gullibility make sense, or can it utilize susceptability already?  Something to think about.
    ;; CB: A high gullibility-threshold of a meme means a turtle has to be extremely gullible to succumb.
    [
    ask link-neighbors with [not resistant? or gullibility > initial-gullibility-threshold]
        [ if random-float 100 < virus-spread-chance
            [ become-infected ] ]
  ]
end

;; CB: Begin spread-resistance addition
;; CB: Critical thinking can also spread, although likely much slower.
;; CB: Don't need to spread resistance to already resistant.
to spread-resistance
  ask turtles with [resistant?]
    [ ask link-neighbors with [not resistant?]
        [ if random-float 100 < resistance-spread-chance
            [ become-resistant ] ] ]
end
;; CB: End addition.
...
to do-virus-checks
  ...
  [
    ;; CB: Note: these chances are less likely to occur.
    ;; CB: We are talking recovery from a mind virus.
    ;; CB: Debate and introspective critical thinking are two recovery methods.
    ;; CB: Assumption: more likely to be convinced via debate than spontaneous critical thinking.
    ;; CB: So, ranges in sliders are reduced compared to natural virus model.
    ...
    ;; CB: Also, critical thinking is likely to be somewhat of a ratchet effect.
    ;; CB: Once resistant, resistance likely to improve over time (especially to similar mind viruses).
    ;; CB: Improve debate and gullibility with some probability.
    ask turtles with [resistant?]
    [
      if random 100 < critical-improvement-prob
      [
        set debate debate + 1
        if debate > 100 [set debate 100]
        set gullibility gullibility - 1
        if gullibility < 0 [set gullibility 0]
      ]
    ]

    ;; CB: Also reinforce host population.  Confirm each other's mind virus.
    ask link-neighbors with [infected?]
    [
      set debate debate - 1
      if debate < 0 [set debate 0]
      set gullibility gullibility + 1
      if gullibility > 100 [set gullibility 100]
    ]
  ]
end
